...
<html>
    ...
    <form action="insertconfirm" method="POST">
        ...
        <input type="text" name="name" value="<%
               if(udb!=null){%><%=udb.getName().trim()%><%}else{
               }
               %>">
        <br><br>
        ...
        <select name="year">
            ...
            <option value="<%=i%>"<%if(udb!=null&&udb.getYear().equals(String.valueOf(i))){%>selected<%}%>> <%=i%> </option>
            <% } %>
        </select>年
        <select name="month">
            <option value="">--</option>
            <%
            for(int i = 1; i<=12; i++){ %>
            <option value="<%=i%>"<%if(udb!=null&&udb.getMonth().equals(String.valueOf(i))){%>selected<%}%>><%=i%></option>
            <% } %>
        </select>月
        <select name="day">
            <option value="">--</option>
            <%
            for(int i = 1; i<=31; i++){ %>
            <option value="<%=i%>"<%if(udb!=null&&udb.getDay().equals(String.valueOf(i))){%>selected<%}%>><%=i%></option>
            <% } %>
        </select>日
        <br><br>
        ...
        <input type="text" name="tell" value="<%
               if(udb!=null){%><%=udb.getTell().trim()%><%}else{
               }
               %>">
        <br><br>
        ...
        <textarea name="comment" rows=10 cols=50 style="resize:none" wrap="hard"><%
               if(udb!=null){%><%=udb.getComment().trim()%><%}else{
               }
               %></textarea><br><br>
